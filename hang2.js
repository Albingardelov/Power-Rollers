...
// Save name and start time in localStorage
function savePlayerInfo() {
    const playerName = prompt("Enter your name:");
    const startTime = new Date().toISOString();
    localStorage.setItem("playerName", playerName);
    localStorage.setItem("startTime", startTime);
    console.log("Player info saved:", playerName, startTime);
}
...
function handleWin() {
    ...
    console.log("Highscore updated:", highscores);
}

// Highscore dialog logic
document.getElementById("highscoreButton").addEventListener("click", function () {
    ...
    highscores.forEach((score, index) => {
        ...
        li.textContent = `${index + 1}. ${score.name} - ${score.score} points (Started: ${score.startTime})`;
        highscoreList.appendChild(li);
    });
    highscoreDialog.showModal();
});

document.getElementById("closeHighscoreDialog").addEventListener("click", function () {
    document.getElementById("highscoreDialog").close();
});
